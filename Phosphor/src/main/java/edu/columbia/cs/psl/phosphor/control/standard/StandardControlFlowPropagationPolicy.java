...
public class StandardControlFlowPropagationPolicy extends AbstractControlFlowPropagationPolicy<StandardControlFlowAnalyzer> {
    ...
    public void visitingIncrement(int var, int shadowVar) {
        ...
        if(Configuration.WITH_TAINT_DEBUG) {
            TaintDebugInstrumenter.instrumentCombineTags(delegate, TaintPassingMV.currentLineNumber, TaintPassingMV.currentClass, TaintPassingMV.currentMethod, TaintPassingMV.currentDesc);
        }
    ...
    private void pushBranchStart() {
        if(nextBranchID != -1) {
            ...
            if(Configuration.WITH_CC_SINKS) {
                SinkInstrumenter.addCCSink(delegate);
            }
